...
export class UsuariosService {
  ...
  login(usuario: string, senha: string): Observable<any>{
    ...
    return this.httpClient.post<any>(this.APILogin,  body, {headers, withCredentials: true})
      .pipe(
        take(1)
      )
  }
  ...
  save(u: Usuario): Observable<Usuario>{
    ...
    if (u.id) {
      return this.httpClient.put<Usuario>(this.API+"usuarios/"+u.id, u, {headers})
    } else{
  ...
  findAll(){
    return this.httpClient.get<Usuario[]>(this.API + 'usuarios')
    .pipe(
      take(1)
    )
  }
